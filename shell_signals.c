...
void eval(char *cmdline) 
{
    ...
    if (builtin_cmd(argv) == 0) {
        ...
        if (ret == 0) { //Child Process
            ...
            if (bg == 1) {
                addjob(jobs, ret, ret, BG, cmdline);
                sigprocmask(SIG_UNBLOCK, &mask, NULL);
                printf("[%d] (%d) %s", pid2jid(ret), ret, cmdline);
            } else {
                addjob(jobs, ret, ret, FG, cmdline);
                sigprocmask(SIG_UNBLOCK, &mask, NULL);
            }
            waitfg(ret);
...
void do_bgfg(char **argv) 
{
    ...
    if (argv[1][0] == '%') {
        ...
        if (strcmp(argv[0], "fg") == 0) {
            ...
            job->state = BG;
            printf("[%d] (%d) %s", jobid, job->pid, job->cmdline);
            kill(-1 * job->pid, SIGCONT);
        }
        return;
    } else {
...
void sigchld_handler(int sig) 
{
    ...
    while((ret = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
        if (WIFSTOPPED(status)) {
            struct job_t *job = getjobpid(jobs, ret);
            job->state = ST;
            printf("Job [%d] (%d) stopped by signal %d\n", pid2jid(ret), ret, WSTOPSIG(status));
        } else if (WIFSIGNALED(status)) {
            int jid = pid2jid(ret);
            deletejob(jobs, ret);
            printf("Job [%d] (%d) terminated by signal %d\n", jid, ret, WTERMSIG(status));
